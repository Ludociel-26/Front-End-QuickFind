  ...
  <div style={{ display: 'flex', alignItems: 'center', minHeight: '24px' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      {item.type === 'item' && item.details?.sku && (
        <span style={{ fontSize: '12px', color: 'gray' }}>{item.details.sku}</span>
      )}
    </div>
  </div>
);
  ...
  },
  {
    ...
    cell: item => (
      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
        {item.tags?.map((tag, i) => (
          <Badge key={i} color={tag.color}>{tag.label}</Badge>
        ))}
      </div>
    ),
    width: 200
...
export const MOCK_FOLDERS: FolderItem[] = [
  {
    ...
    details: { description: "Planta de manufactura.", location: "MTY", manager: "Gerente Ops" },
    children: [
      {
        id: "area-manto",
        ...
        details: { description: "Taller central.", location: "Nave 2", manager: "Jefe Taller" },
        children: [
          {
            id: "rack-elec",
            name: "Estantería Eléctrica",
            type: "folder",
            details: { description: "Componentes eléctricos.", location: "Pasillo 4", manager: "Jefe Taller" },
            children: [
              {
                id: "item-motor",
                name: "Servo Motor 5kW",
                type: "item",
                tags: [{ label: "Siemens", color: "blue" }, { label: "Crítico", color: "red" }],
                details: {
  ...
  },
  {
    ...
    name: "Oficinas CDMX",
    type: "folder",
    details: { description: "Corporativo.", location: "CDMX", manager: "Admin" },
    children: [
      {
        id: "it-closet",
