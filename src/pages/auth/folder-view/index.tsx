...
const DetailRow = ({ label, value }: { label: string, value: React.ReactNode }) => (
  <div>
    <Box variant="awsui-key-label" color="text-label" fontSize="body-s">{label}</Box>
    <Box variant="p" fontSize="body-s" color="text-body-primary">{value}</Box>
  </div>
);
...
export default function FolderInventoryView() {
  ...
  const [splitPanelPreferences, setSplitPanelPreferences] = React.useState({ position: 'side' as const });
  ...
  const { items, actions, filteredItemsCount, collectionProps, filterProps } = useCollection(
    MOCK_FOLDERS,
    {
      ...
      filtering: {
        empty: <Box textAlign="center" color="inherit"><b>No hay datos</b></Box>,
        noMatch: <Box textAlign="center" color="inherit"><b>No hay coincidencias</b></Box>,
        filteringFunction: (item, filteringText) => {
          ...
          const checkMatch = (i: FolderItem): boolean => {
            const matchSelf = i.name.toLowerCase().includes(term) || 
                              (i.details?.sku?.toLowerCase().includes(term) ?? false);
            if (matchSelf) return true;
            if (i.children) return i.children.some(child => checkMatch(child));
            return false;
          };
  ...
  const getSplitPanelContent = (item?: FolderItem) => {
    if (!item) return <Box textAlign="center" padding="l">Selecciona un elemento.</Box>;
        ...
        >
          <Box padding="l">
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '20px' }}>
              <div style={{ padding: '12px', background: 'var(--color-background-container-content)', borderRadius: '8px', border: '1px solid var(--color-border-divider-default)', marginRight: '15px' }}>
            ...
            <ColumnLayout columns={2} variant="text-grid">
              <DetailRow label="Ubicación" value={item.details?.location || "N/A"} />
              <DetailRow label="Responsable" value={item.details?.manager || "N/A"} />
            </ColumnLayout>
            <Box margin={{ top: 'l' }}>
    ...
    return (
      <SplitPanel 
        header={<Header actions={<Button iconName="external" variant="icon">Ficha</Button>}>{item.name}</Header>}
        i18nStrings={{
            preferencesTitle: "Preferencias",
            preferencesPositionLabel: "Posición",
            preferencesPositionDescription: "Ajustar vista",
      ...
      >
        <Box padding={{ bottom: "l" }}>
          <div style={{
            ...
            {item.details?.image ? (
              <img src={item.details.image} alt={item.name} style={{ maxHeight: "100%", maxWidth: "100%", padding: "10px" }} />
          ...
          <ColumnLayout columns={2} variant="text-grid">
            ...
            <SpaceBetween size="m">
              ...
              <div>
                <Box variant="awsui-key-label" fontSize="body-s">Stock</Box>
                <StatusIndicator type={item.details!.stock! > item.details!.min_stock! ? 'success' : 'error'}>
          ...
          <Box margin={{ top: 'm' }}>
             ...
             <Box variant="p" color="text-body-secondary">{item.details?.description}</Box>
          </Box>
          
          <Box margin={{ top: 'l' }}>
            <Button fullWidth variant="primary">Solicitar</Button>
          </Box>
        </Box>
      </SplitPanel>
    );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      <div style={{ flexShrink: 0, zIndex: 1001 }}> <Navbar /> </div>
      ...
      <div style={{ flexGrow: 1, overflow: 'hidden' }}>
        <AppLayout
          ...
          breadcrumbs={
            <RouteTracker 
              items={[ { text: "Almacén", href: "#" }, { text: "Explorador", href: "/folders" } ]}
            />
          }
          content={
            <Box padding={{ top: 's' }}>
              <Table
                ...
                onSelectionChange={({ detail }) => setSelectedItems(detail.selectedItems as FolderItem[])}
