...
export default function NotFound() {
  ...
  return (
    <div
      className={`min-h-screen w-full relative flex items-center justify-center p-4 overflow-hidden ${isDarkMode ? 'bg-[#050505]' : 'bg-gray-200'}`}
    >
      ...
      >
        <div
          className={`absolute inset-0 transition-colors duration-500 ${isDarkMode ? 'bg-black/70' : 'bg-white/30 backdrop-blur-[2px]'}`}
        ></div>
      </div>
      ...
      >
        ...
        >
          ...
          <p
            className={`text-sm md:text-base max-w-md mx-auto ${isDarkMode ? 'text-zinc-400' : 'text-gray-600'}`}
          >
            Parece que la página que buscas se ha movido, eliminado o nunca
            existió en nuestro inventario.
          </p>
        </motion.div>
        ...
        >
          <Link to="/">
            <button
              className={`group relative inline-flex items-center gap-3 px-8 py-4 rounded-2xl font-bold text-sm transition-all duration-300 hover:scale-105 active:scale-95 shadow-lg ${
                isDarkMode
      ...
      {/* Footer Minimalista */}
      <div
        className={`absolute bottom-6 text-center w-full ${isDarkMode ? 'text-zinc-600' : 'text-gray-500'} text-[10px]`}
      >
        &copy; 2026 OmniPart. Todos los derechos reservados.
      </div>
    </div>
  );
}
